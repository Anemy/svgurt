...
function renderLines(svgSettings, lines) {
  ...
  for (i = 0; i < lines.length; i++) {
    ...
    renderString += '<line x1="' + x1 * scale + '" y1="' + y1 * scale + '" x2="' + x2 * scale + '" y2="' + y2 * scale + '" style="stroke: ' + strokeColor + '; stroke-width: ' + strokeWidth + '" />';
  }
...
function createLineAtPoint(x, y, settings, pixelColor) {
  ...
  var lineColor = autoColor ? 'rgb(' + pixelColor.r + ', ' + pixelColor.g + ', ' + pixelColor.b + ')' : strokeColor;
