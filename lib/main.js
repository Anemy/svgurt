...
var _imageManipulator = require('./image-renderer/image-manipulator');
...
function runSvgurtOnFile(config, inputFileName, outputFileName, callback) {
  ...
  (0, _imageSize2.default)(fileNameToImport, function (err, dimensions) {
    ...
    _imageDataUri2.default.encodeFromFile(fileNameToImport).then(function (imageData) {
      var imageDataToUse = [].concat((0, _toConsumableArray3.default)(imageData));
      ...
      (0, _imageManipulator.manipulateImageData)({ data: imageDataToUse }, config, width, height);

      // Run svg creation based on the image data.
      (0, _svgRenderer.renderSvgString)(imageDataToUse, config, width, height, function (svgString) {
