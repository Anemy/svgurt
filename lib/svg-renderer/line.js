...
function createLineAtPoint(x, y, settings, pixelColor) {
  ...
  if (lengthOnColor) {
    lineLength = (0, _color.getPixelColorIntensity)(pixelColor, settings) * length;
  }

  var dir = -direction + 180 * directionRandomness * Math.random();
  var xMove = lineLength * Math.cos(dir * (Math.PI / 180));
  var yMove = lineLength * Math.sin(dir * (Math.PI / 180));
