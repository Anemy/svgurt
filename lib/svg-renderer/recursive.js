  ...
  for (var i = -1; i < 2; i++) {
    for (var k = -1; k < 2; k++) {
      ...
      switch (recursiveAlgorithm) {
        case _Controller.RECURSIVE_LINE_ALGORITHMS.first:
          {
            xMove = xBase + renderEveryXPixels * i;
            yMove = yBase + renderEveryYPixels * k;
            break;
          }
        case _Controller.RECURSIVE_LINE_ALGORITHMS.second:
          {
            xMove = xBase + Math.abs(renderEveryXPixels * i);
            yMove = yBase - renderEveryYPixels * k;
            break;
          }
        case _Controller.RECURSIVE_LINE_ALGORITHMS.third:
          {
            xMove = xBase + Math.abs(renderEveryXPixels * i);
            yMove = yBase - Math.abs(renderEveryYPixels * k);
            break;
          }
        case _Controller.RECURSIVE_LINE_ALGORITHMS.fourth:
          {
            xMove = xBase + renderEveryXPixels * i;
            yMove = yBase + renderEveryYPixels * k;
            break;
          }
        case _Controller.RECURSIVE_LINE_ALGORITHMS.fifth:
          {
            xMove = xBase + Math.abs(renderEveryXPixels * i);
            yMove = yBase + Math.abs(renderEveryYPixels * k);
            break;
      ...
      if (pathAddition) {
        ...
        if (recursiveAlgorithm === _Controller.RECURSIVE_LINE_ALGORITHMS.fifth || recursiveAlgorithm === _Controller.RECURSIVE_LINE_ALGORITHMS.fourth) {
          return pathString;
        }
      }
    }
...
function createRecursivePaths(settings, imageData, width, height) {
  var outputScale = settings.outputScale,
      renderEveryXPixels = settings.renderEveryXPixels,
      renderEveryYPixels = settings.renderEveryYPixels,
